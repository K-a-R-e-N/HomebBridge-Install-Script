...
if [ -n "$(dpkg -l nodejs)" ] || [ -n "$(npm list -g | grep homebridge)" ]; then
echo "Обнаружен установленый пакет"
  while :
  do
  echo " Выберите действие:"
  echo "1. выход из скрипта"
  echo "2. очистить"
  echo "3. продолжить без очистки"
  echo -n "Ввведите 1 или 2 или 3 :"
  read a
  printf "\n"
  case $a in
