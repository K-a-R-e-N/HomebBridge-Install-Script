...
# 3. Инструкция и опросник ввода ПРЯМО В КОНСОЛЬ (без nano)
...
# Ювелирная очистка хвостов: если EOF или знаки двоеточия склеились со строкой Endpoint, убираем их
sudo sed -i 's/EOF://g' /etc/amnezia/amneziawg/awg0.conf
sudo sed -i 's/EOF//g' /etc/amnezia/amneziawg/awg0.conf
sudo sed -i 's/ ://g' /etc/amnezia/amneziawg/awg0.conf
...
# Сужаем AllowedIPs до конкретных подсетей Cloudflare для полной защиты трафика LAN
