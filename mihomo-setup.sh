...
# Полностью вычищаем старые зависшие процессы и службы, если они были
...
if [ ! -f /usr/local/bin/awg ]; then
    # Скачиваем чистое ядро AmneziaWG-GO, работающее без зависимостей
...
# Ювелирно вытаскиваем ключевые параметры для запуска службы напрямую
...
if [ "$AWG_PORT" == "4500" ] || [ "$AWG_PORT" == "2408" ]; then
    # Меняем порт 4500 на заведомо рабочий и не заблокированный в РФ порт 3852
...
# 3. Настройка ультралегкой фоновой службы (Безопасный запуск ядра БЕЗ зависания)
...
Type=simple
User=root
# Запуск ядра напрямую через userspace эмуляцию без awg-quick. Локальная сеть не пострадает!
